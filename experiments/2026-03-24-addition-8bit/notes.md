# 8-Bit Addition Benchmark

This experiment tests whether standard feed-forward and residual ReLU networks can learn exhaustive 8-bit addition from bit-vector inputs.
...
- Targets are the little-endian bits of the exact sum.
- For `bit_width = 8`, the target dimension is `9` to include the carry bit.
- The dataset is exhaustive over all operand pairs, so there are `2^bit_width * 2^bit_width` examples.

### Training parameters

- `steps`: optimizer steps per model.
...
- Residual runs intentionally keep `inter_dim < residual_dim` to act as a bottleneck.
- The default split is random interpolation, not an out-of-distribution generalization test.
...
```bash
uv run python experiments/2026-03-24-addition-8bit/run.py
```