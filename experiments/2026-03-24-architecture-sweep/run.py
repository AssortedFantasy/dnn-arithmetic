"""Compare candidate architectures for 8-bit multiplication."""
...
import jax
import numpy as np
from experimental_models import DenseReluResNet, ReluMLP, ResidualReluMLP
from flax import nnx

from dnn_arithmetic.models import batched_predict
...
class RunConfig:
    ...
    Args:
        ...
        log_every: Training log interval.
        seed: Shared RNG seed for split and initialization.
...
class ExperimentSpec:
    """Architecture configuration for one run.

    Args:
        name: Display name for logs and outputs.
        model_kind: One of "mlp", "residual", or "dense_residual".
        init_output_bias: Whether to initialize the output bias to the target mean.
        hidden_dims: Hidden widths for plain MLP models.
...
def _build_dataset(bit_width: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple of input features and target bit-vectors.
...
def _make_experiments() -> list[ExperimentSpec]:
    """Return the initial architecture sweep."""
    return [
        ExperimentSpec(
            name="mlp-256x4-bias",
            model_kind="mlp",
        ...
        ExperimentSpec(
            name="dense-res-256-b64-x8-bias",
            model_kind="dense_residual",
            init_output_bias=True,
            residual_dim=256,
            inter_dim=64,
            num_blocks=8,
        ...
        ExperimentSpec(
            name="dense-res-256-b128-x8-bias",
            model_kind="dense_residual",
            init_output_bias=True,
            residual_dim=256,
            inter_dim=128,
...
def _build_model_factory(
    ...
    def factory(in_dim: int, out_dim: int, key: jax.Array) -> nnx.Module:
        ...
        elif spec.model_kind == "dense_residual":
            model = DenseReluResNet(
                in_dim=in_dim,
                out_dim=out_dim,
                residual_dim=spec.residual_dim,
                inter_dim=spec.inter_dim,
...
def _count_parameters(model: nnx.Module) -> int:
    """Count trainable parameters in an NNX model.
...
def _format_summary(config: RunConfig, records: list[ExperimentRecord]) -> str:
    """Render a compact text summary for the experiment outputs.

    Args:
        config: Shared run configuration.
        records: Completed experiment records.

    Returns:
        Human-readable summary text.

    """
    best = min(records, key=lambda record: record.final_train_loss)
    lines = [
        (
            "Architecture sweep: 8-bit unsigned multiplication, "
            f"{records[0].spec.name if records else 'n/a'} baseline included"
        ),
        (
            "Training config: "
    ...
    for record in records:
        ...
        lines.extend(
            [
                ...
                (
                    "  train: "
                    f"mse={record.train_metrics.mse:.3e}, "
                ...
                (
                    "  test : "
                    f"mse={record.test_metrics.mse:.3e}, "
...
def _format_loss(value: float | None) -> str:
    """Format an optional loss value for console printing."""
    if value is None:
        return "n/a"
    return f"{value:.3e}"
...
def _make_progress_callback(
    ...
    """Create a console progress callback for one experiment run."""
...
def _parse_args() -> RunConfig:
    """Parse command-line arguments into a typed run configuration."""
...
def main() -> None:
    """Run the architecture comparison sweep and persist outputs."""
    config = _parse_args()
    ...
    for index, spec in enumerate(experiments):
        print(f"Starting {spec.name} ({index + 1}/{len(experiments)})", flush=True)
    ...
    save_line_plot(
        ...
        train_series,
        title="Architecture Sweep Train Loss",
        x_label="Step",
    ...
    save_line_plot(
        TEST_LOSS_PLOT_PATH,
        test_series,
        title="Architecture Sweep Test Loss",
        x_label="Step",
        y_label="Mean Squared Error",
        y_log=True,
    )
    ...
    print(f"Wrote {TRAIN_LOSS_PLOT_PATH}", flush=True)
    print(f"Wrote {TEST_LOSS_PLOT_PATH}", flush=True)
