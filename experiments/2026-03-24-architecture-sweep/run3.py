"""Residual architecture dimension sweep for 8-bit multiplication."""
...
RES_METRICS_PATH = OUTPUT_DIR / "residual_sweep_metrics.json"
RES_SUMMARY_PATH = OUTPUT_DIR / "residual_sweep_summary.txt"
RES_TRAIN_LOSS_PLOT = OUTPUT_DIR / "residual_sweep_train_loss.png"
RES_TEST_LOSS_PLOT = OUTPUT_DIR / "residual_sweep_test_loss.png"

NUM_BLOCKS_VALUES = [4, 6, 8]
RESIDUAL_DIM_VALUES = [128, 256]
INTER_DIM_VALUES = [32, 64, 128]
...
def _make_sweep_specs() -> list[SweepSpec]:
    ...
    for num_blocks, residual_dim, inter_dim in product(
        NUM_BLOCKS_VALUES, RESIDUAL_DIM_VALUES, INTER_DIM_VALUES
    ):
        name = f"res-r{residual_dim}-b{inter_dim}-x{num_blocks}"
        specs.append(
            SweepSpec(
                name=name,
...
def _format_summary(config: RunConfig, records: list[SweepRecord]) -> str:
    ...
    lines = [
        "Residual dimension sweep: 8-bit unsigned multiplication",
        (
        ...
        f"Sweep: num_blocks={NUM_BLOCKS_VALUES}, residual_dim={RESIDUAL_DIM_VALUES}, "
        f"inter_dim={INTER_DIM_VALUES}",
        "",
...
def main() -> None:
    """Run the residual dimension sweep and persist outputs."""
    config = _parse_args()
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    specs = _make_sweep_specs()
    print(
        f"Residual dimension sweep: {len(specs)} configurations, "
    ...
    save_line_plot(
        RES_TRAIN_LOSS_PLOT,
        train_series,
        title="Residual Sweep Train Loss",
        x_label="Step",
    ...
    save_line_plot(
        RES_TEST_LOSS_PLOT,
        test_series,
        title="Residual Sweep Test Loss",
        x_label="Step",
        y_label="Mean Squared Error",
        y_log=True,
    )
    ...
    payload = {
        ...
        "sweep": {
            "num_blocks": NUM_BLOCKS_VALUES,
            "residual_dim": RESIDUAL_DIM_VALUES,
            "inter_dim": INTER_DIM_VALUES,
        },
        "training": asdict(config),
        "results": [asdict(record) for record in records],
    }
    RES_METRICS_PATH.write_text(
        json.dumps(payload, indent=2) + "\n", encoding="utf-8"
    )
    RES_SUMMARY_PATH.write_text(
        _format_summary(config, records), encoding="utf-8"
    )
    print(f"Wrote {RES_SUMMARY_PATH}", flush=True)
    print(f"Wrote {RES_METRICS_PATH}", flush=True)
    print(f"Wrote {RES_TRAIN_LOSS_PLOT}", flush=True)
    print(f"Wrote {RES_TEST_LOSS_PLOT}", flush=True)
