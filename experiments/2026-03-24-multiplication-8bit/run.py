"""Benchmark for learning exhaustive 8-bit unsigned multiplication."""
...
def _write_outputs(
    ...
    summary_lines = [
        (
            f"Benchmark: {args.bit_width}-bit unsigned multiplication, "
...
def main() -> None:
    """Run the 8-bit unsigned multiplication architecture sweep."""
    args = _parse_args()
    ...
    config = TrainingConfig(
        ...
        optimizer=OptimizerConfig(
            ...
            warmup_steps=min(200, max(1, args.steps // 10)),
            constant_steps=min(1000, max(1, args.steps // 3)),
            lr_end_frac=0.05,
            grad_clip_norm=1.0,
        ),
    )
    ...
    print(
        "Benchmark: "
        f"{args.bit_width}-bit unsigned multiplication, {len(split.x_train)} train / {len(split.x_test)} test samples"
    )
    print(
        f"Training config: steps={config.num_steps}, batch_size={config.batch_size}, "
